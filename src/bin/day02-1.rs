...
fn main() -> Result<()> {
    ...
    for line in lines {
        ...
        let (opponent_move, my_recommended_move) = line.split_once(' ').unwrap();
        // Parse the opponent move
        let opponent_move: Move = opponent_move.parse()?;
        // Parse the my recommended move
        let my_recommended_move: Move = my_recommended_move.parse()?;
        debug!(
...
/// Implement FromStr for RockPaperScissors
